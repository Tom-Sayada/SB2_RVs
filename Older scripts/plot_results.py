...
def report_fit_results(
    ...
    # 4) If we have 'plot arrays', use them; else fallback to the same arrays
...
def build_per_epoch_rvs(result, ratio_val, ratio_err):
    ...
    for pk in all_keys:
        if pk.startswith("rv1_epoch"):
            e= int(pk.replace("rv1_epoch",''))
            ep_list.add(e)
        elif pk.startswith("rv2_epoch"):
            e= int(pk.replace("rv2_epoch",''))
            ep_list.add(e)
    ...
    c2b= sum(abs(r['RV2'])>abs(r['RV1']) for _,r in df.iterrows())
    if c2b< (len(df)/2):
        tmp= df.copy()
        df["RV1"]= tmp["RV2"]
        df["RV1_err"]= tmp["RV2_err"]
        df["RV2"]= tmp["RV1"]
        df["RV2_err"]= tmp["RV1_err"]
    return df
...
def plot_epoch_lines(
    ...
    model_flux= compute_full_model(result.params, wv_line_dict, ep_line_dict, central_wavelengths)
...
def plot_rv2_vs_rv1(df_rvs, outdir):
    ...
    axm.errorbar(rv1, rv2, xerr=e1, yerr=e2,
                 fmt='o', color='blue', alpha=0.7, label='Calc RVs')
